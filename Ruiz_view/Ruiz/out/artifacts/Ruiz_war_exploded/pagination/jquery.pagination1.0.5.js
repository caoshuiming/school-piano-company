...
(function($) {
	...
	$.pagMag = {
		...
		if ($pager.reload == 1) {
			if ($.cookie($pager.nameCookie))
				$pager.actualPage = $.cookie($pager.nameCookie);
		}
		...
		if ($pager.actualPage < 2) { // first opening webpage
			...
			if ($pager.num > 0) { // default setting
					...
					if ($pager.max >= 5) {
						...
						for ($pager.j; $pager.j <= $pager.limit; $pager.j++) {
							...
							if ($pager.j == 1) {
								...
								$pager
										.append('<button class="pagerButton actual"><strong>' + $pager.j + '</strong></button> ');

							} else {

								$pager
										.append('<button class="pagerButton">' + $pager.j + '</button> ');
				...
				if ($pager.max >= 5) {
					...
					$pager
							.append('<button class="nextButton">' + '&nbsp;' + '</button> '); // next
																								// (background
																								// image)

				}

				if ($pager.max >= 5 && $pager.actualPage <= $pager.max)
					$pager
							.append('<button class="lastButton">' + $pager.max + '</button> ');

				if ($pager.max > 1)
					$pager
							.append('<button class="allButton">' + '1-' + $pager.max + '</button> '); // if
																										// (actualPage
																										// <=
																										// max)
		...
		} else if ($pager.reload == 1
				&& $.cookie($pager.nameCookie).substr(0, 3) == 'all') { // refreshed
																		// webpage
			...
			$pager.append('<button class="pageButton">' + '分页' + '</button> ');

		} else {

			$.pagMag.pagination($pager.perPage, $pager.reload,
					$pager.actualPage, $pager.max, pager, $pager.nameCookie);

		}

		$(document).on('click',
				'.firstButton',
				function() { // click on first button

					$pager.actualPage = 1;

					$.pagMag.pagination($pager.perPage, $pager.reload,
							$pager.actualPage, $pager.max, pager,
							$pager.nameCookie);
		...
		$(document).on('click',
				'.pagerButton',
				function() { // click on number button
		...
		$(document)
				.on('click', '.allButton', function() { // click on all button
						...
						$pager
								.append('<button class="pageButton">' + '分页' + '</button> ');

						if ($pager.reload == 1)
							var cookieAll = 'all' + $pager.actualPage;

						if ($pager.reload == 1)
							$.cookie($pager.nameCookie, cookieAll); // set
		...
		$(document).on(
				'click',
				'.pageButton',
				function() { // click on pagination button

					$('.pageButton').remove(); // remove page button

					if ($pager.reload == 1)
						$pager.actualPage = $.cookie($pager.nameCookie).substr(
								3);
	...
	pagination : function(perPage, reload, actualPage, max, pager, nameCookie) {
		...
		if ($pager.max > 5) { // max pages
			...
			if ($pager.actualPage > 3)
				$pager
						.append('<button class="firstButton">' + '1' + '</button> '); // 1
																						// button

			if ($pager.actualPage > 1)
				$pager
						.append('<button class="prevButton">' + '&nbsp;' + '</button> '); // prev
		...
		if ($pager.max >= 5) {
			...
			for ($pager.j; $pager.j <= $pager.limit; $pager.j++) { // create
																	// actual
																	// buttons
				...
				if ($pager.j == $pager.actualPage) {
					...
					$pager
							.append('<button class="pagerButton actual"><strong>' + $pager.j + '</strong></button> '); // actual
																														// page

				} else {

					$pager
							.append('<button class="pagerButton">' + $pager.j + '</button> '); // other
																								// pages
		...
		if ($pager.actualPage <= $pager.max && $pager.max > 1)
			$pager
					.append('<button class="allButton">' + '1-' + $pager.max + '</button> ');

		if ($pager.reload == 1)
			$.cookie($pager.nameCookie, $pager.actualPage); // set
